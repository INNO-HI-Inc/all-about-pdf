...
${sec('문의', '개인정보 관련 문의', `개인정보 처리에 대한 문의는 <a href="${rel}contact/">문의 페이지</a>를 통해 접수해 주세요.`)}
  ...
  const html = page({
    ...
    canonical, ogTitle: `개인정보 처리방침 | ${BRAND}`, rel, jsonld: null, main, withScripts: null,
    noChrome: true, bodyClass: 'ws tp', extraScripts: ['assets/js/workspace.js'],
    headExtra: `\n  <script>document.documentElement.className+=" js";</script>\n  <link rel="stylesheet" href="${rel}assets/css/workspace.css?v=${assetVer('assets/css/workspace.css')}">`
  });
  const dir = join(ROOT, 'privacy');
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, 'index.html'), html);
...
${sec('오픈소스', '직접 기여하기', `코드를 보고 함께 개선하고 싶다면 <a href="${escAttr(GITHUB_URL)}" rel="noopener" target="_blank">GitHub 저장소</a>에서 포크·PR로 참여하실 수 있습니다.`)}
  ...
  const html = page({
    ...
    canonical, ogTitle: `문의 · 제안 | ${BRAND}`, rel, jsonld: null, main, withScripts: null,
    noChrome: true, bodyClass: 'ws tp', extraScripts: ['assets/js/workspace.js'],
    headExtra: `\n  <script>document.documentElement.className+=" js";</script>\n  <link rel="stylesheet" href="${rel}assets/css/workspace.css?v=${assetVer('assets/css/workspace.css')}">`
  });
