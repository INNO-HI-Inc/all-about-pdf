...
const GUIDES = ['compress', 'merge', 'password', 'scan', 'imgformat', 'metadata']
  .map((k) => JSON.parse(readFileSync(join(WS, `guides/guide-${k}.json`), 'utf8')));

function guideHref(g, rel) { return `${rel}guide/${encodeURIComponent(g.slug)}/`; }

function buildGuide(g, i) {
  const rel = '../../';
  const canonical = `${SITE_URL}/guide/${encodeURIComponent(g.slug)}/`;
  const secs = g.sections.map((s) => `        <div class="tp-sec" data-reveal><h2 class="tp-h2">${esc(s.h)}</h2>${s.body}</div>`).join('\n');
  const faqs = g.faq.map((f) => `<details><summary>${esc(f.q)}</summary><p>${esc(f.a)}</p></details>`).join('\n          ');
  ...
  const jsonld = [
    { '@context': 'https://schema.org', '@type': 'Article', headline: g.title, description: g.desc, inLanguage: 'ko',
      mainEntityOfPage: canonical, author: { '@type': 'Organization', name: BRAND }, publisher: { '@type': 'Organization', name: BRAND } },
  ...
  const main = `<section class="tp-hero">
      <div class="tp-col">
        ...
        <div class="tp-head">
          <div>
            <h1 class="tp-h1">${esc(g.title)}</h1>
            <p class="tp-sub">${esc(g.desc)}</p>
          </div>
        </div>
      </div>
    </section>
...
function buildGuideIndex() {
  ...
  const main = `<section class="tp-hero">
    ...
    <section class="tp-related" id="related">
      <div class="ws-wrap">
        <div class="ws-sechead" data-reveal>
          <h2><small>가이드</small>무엇이 궁금하신가요?</h2>
          <span class="hint"><span class="dd"></span>모두 무료 · 설치 없이 바로</span>
        </div>
        <div class="tp-relgrid">
        ${cards}
