...
let CUR_LANG = 'ko';
const I18N = {};
try { I18N.en = JSON.parse(readFileSync(join(WS, 'i18n_en.json'), 'utf8')); } catch { I18N.en = null; }
const read = (slug) => {
  if (CUR_LANG !== 'ko') {
    try { return JSON.parse(readFileSync(join(WS, `content_${CUR_LANG}_${slug}.json`), 'utf8')); } catch {}
  }
...
let _enPairs = null;
function enPairs() {
  if (_enPairs) return _enPairs;
  const E = I18N.en, pairs = [];
  const add = (ko, en) => { if (ko != null && en != null && String(ko) !== String(en)) pairs.push([String(ko), String(en)]); };
  ...
  CATEGORIES.forEach((c) => {
    const en = (E.categories || {})[c.id] || '';
    add(c.title, en);
    add('tp-dock__cat-name">' + c.title.replace(/^PDF\s*/, '') + '<', 'tp-dock__cat-name">' + en.replace(/^PDF\s*/i, '') + '<');
    add('ws-cattile__title">' + c.title + '<', 'ws-cattile__title">' + en + '<');
  });
  Object.values(E.optLabels || {}).forEach((o) => add(o.ko, o.en));
  const C = E.chrome || {};
  add('전체 도구', C.allTools);
  add('>소개</a>', '>' + (C.about || 'About') + '</a>');
  add('오픈소스 (GitHub) ↗', C.footGithub); add('오픈소스 · MIT License', C.footLicense); add('오픈소스 ↗', C.openSource);
  add('>인기 도구</h5>', '>' + (C.footPopular || 'Popular tools') + '</h5>');
  add('>이미지 변환</h5>', '>' + (C.footImageConv || 'Image conversion') + '</h5>');
  add('>정보</h5>', '>' + (C.footInfo || 'Info') + '</h5>');
  ...
  add('© 2026 PDF의 모든 것 — made in Korea, runs on your device.', C.footCopyright);
  add('파일 선택', C.chooseFile); add('본문 바로가기', 'Skip to content');
  const dh = C.dropHintSuffix || 'or drag & drop · files stay in your browser';
  add('또는 끌어다 놓기·붙여넣기(여러 개 가능) · 파일은 내 브라우저에서만 처리됩니다', 'or drag, drop or paste (multiple ok) · ' + dh.replace(/^or drag[^·]*· ?/, ''));
  add('또는 끌어다 놓기·붙여넣기 · 파일은 내 브라우저에서만 처리됩니다', 'or drag, drop or paste · ' + dh.replace(/^or drag[^·]*· ?/, ''));
  ...
  add(' 처럼 검색해 보세요.', ' — try another search.');
  add('>파일은 서버로 전송되지 않습니다.<', '>Your files are never uploaded to any server.<');
  add('>참고<', '>Note<');
  add('완료됐어요', 'Done'); add('이어서 해보세요', 'What to do next');
  add('찾는 도구가 없어요', 'No tools found');
  add('압축', 'compress'); add('합치기', 'merge');
  add('이 도구는 자바스크립트가 필요합니다. 브라우저의 자바스크립트를 켜 주세요. 파일은 여전히 서버로 전송되지 않고 내 브라우저에서만 처리됩니다.', C.noscript || 'This tool needs JavaScript. Please enable it — your files are still never uploaded and run only in your browser.');
  add('>다른 도구<', '>' + (C.relHeading || 'More tools') + '<');
  add('다른 PDF 도구도 써보세요', C.relSub || 'Try other PDF tools');
  add('모두 무료 · 설치 없이 바로', C.relHint || 'All free · no install');
  add('>홈<', '>' + (C.breadHome || 'Home') + '<'); add('>도구<', '>' + (C.breadTools || 'Tools') + '<');
  add('처음으로 ✕', C.backToStart || 'Start over ✕');
  ['PDF', '이미지', 'JPG', 'PNG', 'WEBP', 'GIF', 'AVIF', 'SVG'].forEach((n) => {
    const en = n === '이미지' ? 'images' : n;
    add(`여기에 ${n}를 놓고 바로 작업`, `Drop your ${en} here to get started`);
    add(`여기에 ${n}을 놓고 바로 작업`, `Drop your ${en} here to get started`);
  });
  ...
  add('자주 묻는 질문', C.homeFaqH || 'Frequently asked questions');
  ...
  _enPairs = pairs.sort((a, b) => b[0].length - a[0].length);
  return _enPairs;
}

function langSwitch(canonical, lang) {
  if (!canonical) return '';
  const path = canonical.replace(SITE_URL, '') || '/';
  const enHref = '/en' + (path === '/' ? '/' : path);
  const other = lang === 'en' ? { href: path, label: '한국어', hl: 'ko' } : { href: enHref, label: 'EN', hl: 'en' };
  return `<a class="ws-langswitch" href="${other.href}" hreflang="${other.hl}" aria-label="change language">${other.label}</a>`;
}

function localizeEn(html, canonical) {
  if (!I18N.en) return html;
  let out = html;
  for (const [ko, en] of enPairs()) if (out.indexOf(ko) >= 0) out = out.split(ko).join(en);
  // 검색 키워드(data-keywords)의 남은 한글 제거 — 영어 검색 오염·비가시 한글 방지
  ...
  if (canonical) {
    ...
    out = out.replace('rel="canonical" href="' + canonical + '"', 'rel="canonical" href="' + enC + '"')
...
function emitPage(subPath, html, canonical) {
  ...
  writeFileSync(outPath, CUR_LANG === 'ko' ? koPost(html, canonical) : localizeEn(html, canonical));
}
// about/privacy/contact 영어 본문을 i18n_en.json.pages 구조로 생성(프로즈는 사전치환이 아닌 구조 스왑).
function enPageMain(key, rel) {
  const p = I18N.en && I18N.en.pages && I18N.en.pages[key];
  if (!p) return null;
  ...
  return `<section class="tp-hero">
      <div class="tp-col">
        <nav class="tp-path" aria-label="location"><a href="${rel}">Home</a><span class="s" aria-hidden="true">/</span><b>${esc(p.h1)}</b></nav>
        <div class="tp-head">
...
const enOr = (key, rel, koMain) => (CUR_LANG === 'en' && I18N.en && I18N.en.pages && I18N.en.pages[key]) ? enPageMain(key, rel) : koMain;
...
${sec('호스팅 로그', '서버 로그', '이 사이트는 GitHub Pages로 호스팅됩니다. 호스팅 제공자는 서비스 제공·보안을 위해 접속 IP·브라우저 종류 등 표준 접속 로그를 일시적으로 처리할 수 있습니다. 자세한 내용은 GitHub의 개인정보 정책을 따릅니다.')}
...
${sec('제3자 링크', '외부 링크', '이 사이트에는 GitHub 등 외부 사이트로 향하는 링크가 있을 수 있습니다. 외부 사이트의 개인정보 처리에는 이 방침이 적용되지 않으며, 각 사이트의 정책을 확인해 주세요.')}
...
${sec('버그 · 제안', '기능 제안 · 버그 제보', `「${BRAND}」은 오픈소스입니다. 버그나 개선 아이디어는 <a href="${escAttr(GITHUB_URL)}/issues" rel="noopener" target="_blank">GitHub 이슈</a>에 남겨주시면 가장 빠르게 반영됩니다.`)}
${sec('오픈소스', '직접 기여하기', `코드를 보고 함께 개선하고 싶다면 <a href="${escAttr(GITHUB_URL)}" rel="noopener" target="_blank">GitHub 저장소</a>에서 포크·PR로 참여하실 수 있습니다.`)}
      </div>
    </section>`;
  const html = page({
    title: `문의 · 제안 | ${BRAND}`,
...
function buildSeoFiles() {
  ...
  const entries = [
    ...
    ...(I18N.en ? [
      { url: SITE_URL + '/en/', file: 'en/index.html', priority: '0.9' },
      ...TOOLS.map((t) => ({ url: `${SITE_URL}/en/${t.slug}/`, file: `en/${t.slug}/index.html`, priority: '0.7' })),
      { url: SITE_URL + '/en/about/', file: 'en/about/index.html', priority: '0.4' },
      { url: SITE_URL + '/en/privacy/', file: 'en/privacy/index.html', priority: '0.2' },
      { url: SITE_URL + '/en/contact/', file: 'en/contact/index.html', priority: '0.2' }
    ] : [])
  ];
...
const BUILD_LANGS = I18N.en ? ['ko', 'en'] : ['ko'];
