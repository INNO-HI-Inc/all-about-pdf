...
const LDIR = {
  ...
  ja: { fmtLabel: '対応形式', dropRoute: 'PDFをドロップするとページ整理、画像をドロップすると画像からPDFが開きます · ファイルがサーバーに送られることはありません。', lead: '必要な作業を選べば、すぐに実行できます。すべてのツールはインストールも登録も不要で無料。ファイルはブラウザの中だけで処理されます。', more: '37のツールをすべて見る', dropAny: 'ここにファイルをドロップ',
  ...
  zh: { fmtLabel: '支持格式', dropRoute: '拖入 PDF 打开整理页面，拖入图片打开图片转 PDF · 文件不会上传到服务器。', lead: '选择需要的操作即可立即执行。所有工具均免费，无需安装或注册，文件仅在您的浏览器中处理。', more: '查看全部 37 个工具', dropAny: '将文件拖到此处',
